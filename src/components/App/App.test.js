...
describe('<App />', () => {
  ...
  it('User should be able to input an intention and draw a card', async () => {
    ...
    fetchSingleCard.mockResolvedValue(card)
    fetchThreeCards.mockResolvedValue(threeCards)
    ...
    const drawThreeCardsButton = getByText('3 CARD SPREAD')
    ...
    await waitFor(() => {fireEvent.click(drawThreeCardsButton)})
    expect(getByText('The Magician')).toBeInTheDocument()
